...
// Verifies the hard-lock: ore recovery = 1% of mining speed
...
describe('ore recovery invariant', () => {
  it('ORE_RECOVERY_RATIO is hard-locked to 1%', () => {
    expect(ORE_RECOVERY_RATIO).toBe(0.01)
  })
  ...
  it('ORE_REGEN_RATE is exactly 1% of ORE_HARVEST_RATE', () => {
    expect(ORE_REGEN_RATE / ORE_HARVEST_RATE).toBe(0.01)
  })
  ...
  it('GameMap ore recovery has no local override paths (no adjacentBonus)', () => {
    ...
    // adjacentBonus regen override must NOT be present (would conflict with 1% spec)

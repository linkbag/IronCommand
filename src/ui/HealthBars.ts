...
export class HealthBars {
  ...
  draw(entities: EntityForHP[], camOffX: number, camOffY: number, localPlayerId = 0) {
    ...
    for (const e of entities) {
      ...
      const screenPos = cartToScreen(e.x, e.y)
      const sx = screenPos.x - this.BAR_W / 2
      const sy = screenPos.y + this.BAR_OFF_Y
      ...
      if (rank > 0 && this.selectedIds.has(e.id)) {
        this.drawChevrons(g, screenPos.x, sy - this.CHEV_H - this.CHEV_GAP, rank)
      }
    }
  }

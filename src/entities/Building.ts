...
export class Building extends Phaser.GameObjects.Container {
  ...
  private getBuildingPalette(pct: number, visualType: FacilityVisualType): BuildingPalette {
    ...
    return {
      ...
      symbol: adjustBrightness(main, 54),
      warning: 0xf29b3a,
    }
  }
  ...
  private drawBuildingDetails(
    ...
    const hw = dims.halfW
    const hh = dims.halfH
    const badgeY = dims.topY + hh * 0.14
    const badgeW = Math.max(7, hw * 0.26)
    const badgeH = Math.max(5, hh * 0.34)
    const roofBadge = [
    ...
    this.drawFacilitySymbol(g, visualType, 0, badgeY, Math.max(6, Math.round(hw * 0.22)), palette.symbol, palette.line)
  ...
  private drawFacilitySymbol(
    ...
    const half = Math.max(3, Math.round(size / 2))
